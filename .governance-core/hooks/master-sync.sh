...
for h in .claude/hooks/*.sh governance-core/hooks/*.sh; do [ -e "$h" ] || continue; bash -n "$h" 2>/dev/null || { echo "🔸 master-sync: $h com erro de sintaxe -> nao sincronizo."; exit 0; }; done
...
# GATE 6b: governanca de deploy coerente com docs/DEPLOY_AUTONOMY.md (read-only, stdlib, fail-closed)
if [ -f scripts/asb_audit_deploy_governance.py ]; then
  python3 scripts/asb_audit_deploy_governance.py >/dev/null 2>&1 || { echo "🔸 master-sync: governanca de deploy incoerente (rode scripts/asb_audit_deploy_governance.py) -> nao sincronizo."; exit 0; }
fi
