...
class SecretGate:
    def __init__(self, root):
        self.g1 = None
        self.supp = None
        # FONTE UNICA = o Guardiao 1 do core. Ordem: vendorizado (satelite) -> canonico -> legado.
        guards = [os.path.join(root, ".governance-core", "hooks", "preflight-gate.sh"),
...
def main():
    ...
    sec_abs = os.path.realpath(os.path.join(root, ".asb", "security.json"))
    ws_dir = os.path.realpath(os.path.join(root, ".asb", "workstreams"))
    find_dir = os.path.realpath(os.path.join(root, ".asb", "security-findings"))
    if tgt_abs == proj_abs:
        kind = "project"
    elif tgt_abs == sec_abs:
        kind = "security-profile"
    elif tgt_abs.startswith(ws_dir + os.sep):
        kind = "workstream"
    elif tgt_abs.startswith(find_dir + os.sep):
        kind = "security-finding"
    else:
        die(3, "ERRO: alvo nao e .asb/project.json, .asb/security.json, "
               ".asb/workstreams/<id>.json nem .asb/security-findings/<id>.json")
    # FONTE UNICA = schema do core. Ordem: vendorizado (satelite) -> canonico -> legado.
    for _cand in (os.path.join(root, ".governance-core", "schema", kind + ".schema.json"),
                  os.path.join(root, "governance-core", "schema", kind + ".schema.json"),
                  os.path.join(root, ".asb", "schema", kind + ".schema.json")):
        schema_path = _cand
    ...
    # coerencia do finding: risco aceito em HIGH/CRITICAL SEM aprovacao humana registrada nao vale.
    # Sem esta checagem, bastaria escrever accepted_risk no arquivo para o gate parar de bloquear —
    # a decisao humana viraria um campo de texto (contrato de seguranca, secao 3).
    if kind == "security-finding" and isinstance(inst, dict):
        if inst.get("status") == "accepted_risk" and inst.get("severity") in ("high", "critical"):
            ha = inst.get("human_approval")
            if not isinstance(ha, dict) or not ha.get("approver"):
                errs.append("accepted_risk em severity high/critical exige human_approval registrada")
        if inst.get("status") == "closed" and not inst.get("closed_at"):
            errs.append("status 'closed' sem closed_at")
        fname = os.path.basename(tgt_abs)
        fid = inst.get("finding_id")
        if isinstance(fid, str) and fname != fid + ".json":
            errs.append(f"filename '{fname}' != '{fid}.json'")

    if kind == "security-profile" and isinstance(inst, dict):
        # `profile` e o PISO de risco do repositorio: o quao ruim e um erro aqui, em qualquer
        # arquivo. `surface` documenta o que existe de sensivel — e o classificador ja eleva o
        # risco quando o caminho tocado e um desses. Por isso a lista abaixo e so o subconjunto
        # onde um erro em QUALQUER lugar do repo ja e grave; exigir piso alto por qualquer
        # superficie tornaria todo repo HIGH, e um gate que sempre bloqueia ensina a ignorar gate.
        piso_alto = {"auth", "authz", "secrets", "rls", "remote_execution", "sandbox",
                     "crypto", "infrastructure"}
        if set(inst.get("surface") or []) & piso_alto and inst.get("profile") in ("low", "medium"):
            errs.append("surface de fronteira de confianca exige profile 'high' ou 'critical'")
