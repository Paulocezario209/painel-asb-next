...
allowed-tools: Bash(bash:*), Bash(cd:*), Bash(git fetch:*), Bash(git rev-parse:*), Bash(timeout:*), Bash(echo:*), Bash(python3 scripts/asb_render_brief.py:*)
---
...
Resolve a raiz do repo de forma DETERMINÍSTICA e FAIL-CLOSED (prefere `CLAUDE_PROJECT_DIR` válido;
senão evidência Git/projeto; ambiguidade **falha**, nunca roda no repo errado), atualiza as refs
(sem pull/merge; falha de rede = `FETCH_STATUS=UNKNOWN`, nunca sucesso) e renderiza o estado real —
tudo a partir da MESMA raiz resolvida:

!`ROOT="$(s=scripts/asb_resolve_root.sh; f=""; for c in "${CLAUDE_PROJECT_DIR:-}" "$(git rev-parse --show-toplevel 2>/dev/null)" ./*/ .; do [ -n "$c" ] && [ -f "${c%/}/$s" ] && { f="${c%/}/$s"; break; }; done; [ -n "$f" ] && bash "$f")" && cd "$ROOT" && { GIT_TERMINAL_PROMPT=0 timeout 8 git fetch origin --prune --quiet >/dev/null 2>&1 && echo FETCH_STATUS=OK || echo FETCH_STATUS=UNKNOWN; } && python3 scripts/asb_render_brief.py || echo "ASB_BOOT_FALHOU: raiz do repo nao resolvida (fail-closed) — rode /asb de dentro do repo ou defina CLAUDE_PROJECT_DIR valido."`
