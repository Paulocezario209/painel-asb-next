...
import { bucketByOrders, isViva, filterByView, JORNADA_STAGES, type JornadaView, type JornadaStageKey } from "@/lib/funnel/jornada";
...
export default async function JornadaDrillPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const scoped = filterByView(clientes, view).filter((c) => bucketByOrders(c.total_orders) === stage);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <PageHead
        ...
        desc={`Base: ${view === "viva" ? "Carteira Viva (ativo/atenção)" : "Histórico Geral (toda a carteira faturada)"} · ${rows.length} clientes · ${brl(totalReceita)} faturado · classificação por histórico completo`}
      />
