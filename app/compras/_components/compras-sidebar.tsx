...
export function ComprasSidebar({ isOpen = false, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  ...
  return (
    ...
    >
      <Link href="/inicio" title="Voltar à frente institucional" style={{ display: "flex", alignItems: "center", gap: 12, textDecoration: "none", padding: "18px 14px 16px", borderBottom: `1px solid ${SB.border}` }}>
        <div style={{ width: 42, height: 42, borderRadius: 12, flexShrink: 0, background: `linear-gradient(140deg, ${GREEN} 0%, #1e7a34 45%, #1B2A6B 100%)`, display: "grid", placeItems: "center", color: "#fff", fontWeight: 850, fontSize: 15, letterSpacing: ".5px", boxShadow: "0 8px 18px -8px rgba(46,160,67,.5)" }}>ASB</div>
        <div style={{ lineHeight: 1.15 }}>
          <div style={{ fontWeight: 750, fontSize: 15, color: "#fff", fontFamily: SANS, letterSpacing: "-.01em" }}>American Steak</div>
          <div style={{ fontSize: 10.5, color: "#8b93a7", fontWeight: 600, fontFamily: SANS, marginTop: 1 }}>Compras & Estoque</div>
        </div>
      </Link>
      ...
      <nav className="flex-1 px-2 py-3">
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon style={{ width: 17, height: 17, flexShrink: 0, color: isActive ? GREEN : undefined }} />
      ...
      <div className="px-4 py-3" style={{ borderTop: `1px solid ${SB.border}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <p style={{ color: "#8b93a7", fontSize: 10, letterSpacing: ".06em", fontWeight: 600, fontFamily: SANS }}>Compras · Fase 0</p>
        </div>
      </div>
    </aside>
  );
