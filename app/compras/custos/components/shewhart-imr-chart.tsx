...
export function ShewartIMRChart({ data, unit = "", title }: {
  ...
  if (serie.length < 2) {
    return <div style={{ ...sCard, padding: 16 }}><p style={{ ...sLabel, marginBottom: 6 }}>{title}</p><p style={{ color: C.mut, fontSize: 11, fontFamily: mono }}>São necessários pelo menos 2 pontos com dados.</p></div>;
  }
  ...
  return (
    <div style={{ ...sCard, padding: 14 }}>
      <p style={{ color: C.branco, fontSize: 12, fontWeight: 700, fontFamily: mono, letterSpacing: ".06em", marginBottom: 4 }}>{title}</p>
      <p style={{ color: r.pontosFora > 0 ? C.vermelho : r.pontosAtencao > 0 ? C.amarelo : C.verde2, fontSize: 10, fontFamily: mono, marginBottom: 10 }}>● {r.sinal}</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6, marginTop: 10 }}>
        {[["Média", fmt(L.media)], ["σ", fmt(L.sigma)], ["Mín", fmt(L.min)], ["Máx", fmt(L.max)], ["LCS", fmt(L.lcs)], ["LCI", fmt(L.lci)], ["Fora", String(r.pontosFora)], ["Atenção", String(r.pontosAtencao)]].map(([k, v]) => (
          <div key={k} style={{ background: C.card2, borderRadius: 4, padding: "6px 8px" }}>
            <p style={{ fontSize: 8, color: C.mut2, fontFamily: mono, textTransform: "uppercase" }}>{k}</p>
            <p style={{ fontSize: 12, color: C.texto, fontFamily: mono, fontWeight: 700 }}>{v}</p>
          </div>
        ))}
      </div>
    </div>
